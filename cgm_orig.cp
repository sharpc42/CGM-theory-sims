...
// Are we able to wrap off and modularize some of this code and import with user headers?

using namespace std;   // in point of fact I'm not *using* this right now at all...
...
Real m_H   = 8.42e-58;					    // in solar masses
      ...
      for (int j = js; j <= je; ++j) {
        for (int i = is; i <= ie+1; ++i) {
          if (COORDINATE_SYSTEM == "cartesian") {
            ...
            //pfield->b.x2f(k,j,i) = b0 * modeSum * std::sin(angle);
            
            // Only Cartesian supported for now; some way to handle general conversion in the future?
          } else if (COORDINATE_SYSTEM == "cylindrical") {
